  ...
  create: protectedProcedure.input(OrderCreateSchema).mutation(async ({ ctx, input }) => {
    ...
    const order = await ctx.db.$transaction(async (tx) => {
      ...
      for (const item of input.items) {
        ...
        // If no inventory record exists, allow — admin tracks manually
      }
    ...
    // A CAPI failure will never roll back the order.
    const addr = order.shippingAddress as { fullName?: string; phone?: string; city?: string; province?: string; postalCode?: string } | null;
